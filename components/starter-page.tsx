...
const useStyles = makeStyles({
  ...
  ideaDescription: { minHeight: '180px' },
...
export function StarterPage() {
  ...
  function submitIdea(event: React.FormEvent) {
    ...
    setSubmittedIdea(newTitle.trim())
    setTicketNumber(`FWD-${String(Date.now()).slice(-6)}`)
    setSubmitted(true)
    setNewTitle('')
    setNewDescription('')
    setShowNew(false)
  ...
  return (
    <div className={styles.page}>
      <div className={styles.shell}>
        ...
        <div className={styles.content}>
          ...
          <main className={styles.main}>
            ...
            {submitted && <MessageBar intent="success"><MessageBarBody><strong>Idea shared.</strong> “{submittedIdea}” is now logged in Capture with ticket <strong>{ticketNumber}</strong>.</MessageBarBody><Button appearance="transparent" icon={<Dismiss24Regular />} onClick={() => setSubmitted(false)} aria-label="Dismiss notification" /></MessageBar>}
      ...
      {showNew && <aside aria-label="Share a new idea" className={styles.drawerOverlay} onClick={() => setShowNew(false)}><div className={styles.drawer} onClick={(event) => event.stopPropagation()}><div className={styles.drawerHeader}><div><Badge appearance="tint" color="brand">New opportunity</Badge><Title2 as="h2">Share an idea</Title2></div><Button appearance="subtle" icon={<Dismiss24Regular />} onClick={() => setShowNew(false)} aria-label="Close share idea form" /></div><form className={styles.form} onSubmit={submitIdea}><Field label="What idea do you have?" required><Input value={newTitle} onChange={(event) => setNewTitle(event.target.value)} placeholder="Give your idea a clear title" /></Field><Field label="Tell us a little more"><Textarea className={styles.ideaDescription} value={newDescription} onChange={(event) => setNewDescription(event.target.value)} placeholder="What could be better, faster or simpler?" /></Field><div className={styles.formActions}><Button type="button" appearance="secondary" onClick={() => setShowNew(false)}>Cancel</Button><Button type="submit" appearance="primary" icon={<CheckmarkCircle24Regular />}>Share idea</Button></div></form></div></aside>}
    </div>
  )
}
